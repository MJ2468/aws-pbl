...
def write():
    if request.method == 'POST':
        ...
        if error is None:
            db = pymysql.connect(host='database-1.cxgs4lmxkchl.ap-northeast-2.rds.amazonaws.com',port=3306,db='awsdb',passwd=db_password,user='admin')
            cursor = db.cursor()
            cursor.execute(
            ...
            if file :
                ...
                cursor.execute(
                    ...
                    (board_id, file.filename,  's3-board-files', 'images/' + file.filename,  file.content_type)
                )
            db.commit()
            cursor.close()
            db.close()
